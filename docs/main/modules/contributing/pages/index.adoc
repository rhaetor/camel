...
** Use the plain Camel Core (that is:  if it's a problem on Camel Core, avoid creating a reproducer based on Camel Quarkus, Camel Spring Boot and others).
...
- simulate the infrastructure required for the test (that is, JMS brokers, Kafka, etc),
...
* Less is more:
** Avoid creating unnecessary Maven profiles (that is, to enable/disable tests)
...
creating the contribution (that is, the code on your fork) is too outdated compared to the current version in the Camel upstream repository.
