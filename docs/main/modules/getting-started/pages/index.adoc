...
[NOTE]
====
We strongly recommend you use an LTS version of Java (that is, 11 or 17). We regularly test Camel in our continuous integration (CI) environment using Java's LTS versions.
====
...
* `-DarchetypeArtifactId`: this is the ID of the archetype to use (that is, which of the templates from Camel Core to use - `camel-archetype-java` in this case)
...
A URN is a wrapper for different "unique identifier" schemes. The syntax of a URN is `urn:<scheme-name>:<unique-identifier>`. A URN uniquely identifies a _resource_ (that is: , a book, person, or piece of equipment). By itself, a URN does not specify the _location_ of the resource.
