...
parameter value to be *raw* using the following syntax `RAW(value)`, for example
...
lines as its just Java code, for example, just concat the `String`. When using XML
DSL then the URI is an attribute, for example, `<from uri="bla bla"/>`. From Camel
