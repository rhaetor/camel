...
that is, `com.foo.MyOrder` should now be `com.foo.MyOrder.class`.
...
Property names of blob fields on generated DTOs no longer have 'Url' affixed. For example,  the `ContentVersionUrl` property is now just `ContentVersion`.
