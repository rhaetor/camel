...
*  Use a three digit release version of the form: `MAJOR.MINOR.PATCH`, for example, `3.0.0`.

*  For the tag use a string of the form: `camel-MAJOR.MINOR.PATCH`, for example, `camel-3.0.0`.

*  For the next version increase the patch version and append `-SNAPSHOT`, for example, `3.0.1-SNAPSHOT`.
...
. Merge the release branch back into the corresponding base branch (for example, merge `release/3.2.0` into `camel-3.2.x`)
