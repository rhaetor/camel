...
You can also use wildcards (that is, `*`) to match multiple files, such as running all the yaml files:
...
[source,bash]
----
2023-12-15 21:46:44.218  INFO 15033 --- [           main] org.apache.camel.main.MainSupport   : Apache Camel (JBang) 4.4.2-SNAPSHOT is starting
...
2023-12-15 21:46:55.239  INFO 15033 --- [           main] el.impl.engine.AbstractCamelContext : Apache Camel 4.4.2-SNAPSHOT (foo) is starting
...
Which then will send the message to the incoming endpoint in the route, that is, `jms:inbox` which has been stubbed.
...
You can only specify one profile name, that is, `--profile=local,two` is not valid.
...
You can also use wildcards (that is, `*`) to match multiple files, such as running all the groovy files:
...
the _from_ endpoint, that is, `kamelet:mqtt5-source` with all its configuration. If there are multiple routes,
...
You can find multiple items by repeating the option, that is:
...
Here we can see the circuit breaker is in _half open_ state, that is, a state where the breaker is attempting
...
2023-11-03 18:15:06.833  INFO 33384 --- [           main] el.impl.engine.AbstractCamelContext : Apache Camel 4.4.2-SNAPSHOT (foo) is starting
...
2023-11-03 18:15:06.924  INFO 33384 --- [           main] el.impl.engine.AbstractCamelContext : Apache Camel 4.4.2-SNAPSHOT (foo) started in 91ms (build:0ms init:0ms start:91ms)
...
This will then automatic insert or update the JBang depencies (`//DEPS`) in the top of the source file.
...
The jib-maven-plugin must be configured with a from and to image. The from image will default be `eclipse-temurin:17-jre`
for Java 17 and `eclipse-temurin:21-jre` for Java 21. However, companies often have their own base images to be used, so
you can configure this in the properties (together with authentication if needed):
...
Resource fragments for JKube allows to store external configuration in YAML resource descriptions, which
...
You can find more information about resource fragments at the https://eclipse.dev/jkube/docs/kubernetes-maven-plugin/#_resource_fragments[Eclipse JKube] website.
...
To be able to continue to use Camel CLI (that is, `camel`), you need to add `camel:cli-connector` in the `--deps` option, as shown below:
...
|Resource fragments for Kubernetes (Use commas to separate multiple files). See more details at xref:_exporting_to_camel_main_for_kubernetes[].
