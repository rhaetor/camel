...
You can return a `CompletionStage` implementation (for example, `CompletableFuture`)
...
xref:components::bean-component.adoc[Bean] to handle, for example, an `onException` route.
...
means you can use, for example, `$\{body}`, `${header.foo}` and others
...
By default, Camel will match the type name using the simple name, for example
