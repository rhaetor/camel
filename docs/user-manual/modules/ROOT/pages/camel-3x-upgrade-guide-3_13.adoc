...
The `KafkaManualCommit` class were modified to support async manual commit. Please use the new function `commit()` instead of the old deprecated one `commitSync()`.
...
This is that is, needed when using `camel-servlet` which runs under a specific `context-path`.
