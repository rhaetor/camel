...
The component auto determines what flow you're trying to configure. To be explicit, set the
...
| `sObjectName` | `String` | Name of SObject, for example, `Account`. Alternatively, can be supplied in Body. | | x
|===
...
| `sObjectName` | `String` | Name of SObject, for example, `Account` | | x
...
| `sObjectName` | `String`  | Name of SObject, for example, Account | | Required if SObject not supplied in body
...
| `sObjectName` | `String` | Name of SObject, for example, `Account`. Only used if Camel cannot determine from Body. | | If Body is a `String`
|===
...
| `sObjectName` | `String` | Name of SObject, for example, `Account`. Only used if Camel cannot determine from Body. | | If Body is not an `AbstractSObjectBase` instance
...
| `sObjectClass` | `String` | Fully qualified name of class to deserialize response to. Usually a subclass of `AbstractQueryRecordsBase`, for example, `org.my.dto.QueryRecordsAccount`| |
| `sObjectName` | `String` | Simple name of class to deserialize response to. Usually a subclass of `AbstractQueryRecordsBase`, for example, `QueryRecordsAccount`. Requires the `package` option be set.| |
|===
...
`java.util.Map` as value. Requests are made in JSON format hold some type information (that is, it is known what values are
...
the result will be a `java.util.Map` with string keys and values or other `java.util.Map` as value. Requests are made in JSON format and hold some type information (that is, it is known what values are strings and what values are numbers).
====
...
| sObjectName | String | Type of SObject, for example, `Account` | | x
...
In either case the Apex URL can contain placeholders in the format of `\{headerName}`. For example,  for the Apex URL `MyApexClass/\{id}`,
...
| `apexUrl` | `String` | The portion of the endpoint URL after `https://instance.salesforce.com/services/apexrest/`, for example, 'MyApexClass/' | | Yes, unless supplied in endpoint
| `apexMethod` | `String` | The HTTP method (for example, `GET`, `POST`) to use. | `GET` |
...
| `sObjectName` | `String` | Name of sObject (for example, `Merchandise__c`) used to deserialize the response |  |
...
that is, xml, csv, zip/xml, and zip/csv are supported. +
 The request and response have to be marshaled/unmarshaled by the
...
* Avro `GenericRecord`. Camel fetches the Avro schema to serialize `GenericRecord`
...
* POJO. Camel fetches the Avro schema to serialize POJO instances. The POJO's
...
for example, setting the `id` property, which can be tied back to the `PublishResult.CorrelationKey`.
...
To emit a platform event use the <<createSObject,createSObject>> operation, passing an instance of a platform event, for example, `Order_Event__e`.
...
| `rawPath` | `String` | The portion of the endpoint URL after the domain name, for example, '/services/data/v51.0/sobjects/Account/' | | x
