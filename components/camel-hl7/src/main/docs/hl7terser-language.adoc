...
Furthermore, when unmarshaling the HL7 data format forwards the configured HAPI context in
...
// customize HapiContext some more ... for example, enforce that PID-8 in ADT_A01 messages of version 2.4 is not empty
...
message as a response to an incoming HL7v2 message, for example, based on a
