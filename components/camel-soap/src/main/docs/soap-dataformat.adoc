...
prefix mappings when marshaling using SOAP data format.
...
marshalnormal objects as well as exceptions.
...
[source,xml]
-----------------------------------------------------------------------------------------------------
    <!-- Defining a ServiceInterfaceStrategy for retrieving the element name when marshaling -->
...
The following route supports marshaling the request and unmarshaling a
response or fault.
