...
- Provide an implementation of `com.azure.core.credential.TokenCredential` into the Camel's Registry, for example, using the `com.azure.identity.DefaultAzureCredentialBuilder().build();` API.
...
For example, to consume events from EventHub, use the following snippet:
...
Also, the component supports *aggregation* of messages by sending events as *iterable* of either Exchanges/Messages or normal data (for example, list of Strings). For example:
