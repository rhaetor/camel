...
of operation being requested on the queue level, for example, `createQueue, `sendMessage`, and so on, both `accountName` and `queueName` are required.
...
from("direct:start")
    .process(exchange -> {
      ...
      // for example, to only returns a list of queues with 'awesome' prefix:
