...
protocol implementation (for example, send a MIME-multipart over an HTTP
endpoint) or as a kind of tunneling solution (for example, because camel-jms
does not support attachments but by marshaling the message with
attachments into a MIME-Multipart, sending that to a JMS queue,
receiving the message from the JMS queue and unmarshaling it again
(into a message body with attachments).

The marshaloption of the mimeMultipart data format will convert a
...
The unmarshaloption of the mimeMultipart data format will convert a
...
be set as Camel headers. The unmarshaling will only take place if the
