...
value pair fields (for example, _FIX messages_).

To work with camel-bindy, you must first define your model in a package
(for example, `com.acme.model`) and for each model class (for example, `Order`, `Client`,
...
* Type of record (CSV, key value pair (for example, _FIX message_), fixed length
...),
...
[width="100%",cols="15%s,15%,60%",options="header",]
|===
...
| removeQuotes | boolean |  | true | The remove quotes parameter flags if unmarshaling should try to remove quotes for each field
...
as delimiter (for example, '10', 'Street 10, NY', 'USA' or "10", "Street 10,
NY", "USA").

[NOTE]
====
In this case, bindy will remove the first and last character of the line which is a single or double quotes.
====
...
record is enclosed with either single or double quotes and automatically removes those quotes when unmarshaling from CSV to Object. Therefore, do
*not* include the quotes in the separator, but do as below:
...
For example: If the model class Client is linked to the Order class, then use
...
(that means that we have an asymmetric process comparing marshaling
from unmarshaling) then we can use `position` to indicate this.
...
If a field is mandatory, use the attribute `required` set to
true.
...
before they are processed, use the attribute `trim` set to
true.
...
| eol | String |  |  a| Character to be used to process considering end of line after each record while unmarshaling (optional -
...
| ignoreTrailingChars | boolean |  | false a| Indicates that characters beyond the last mapped filed can be ignored when unmarshaling / parsing. This
...
| skipFooter | boolean |  | false a| Configures the data format to skip marshaling / unmarshaling of the footer record. Configure this parameter on
the primary record (for example, not the header or footer).

| skipHeader | boolean |  | false a| Configures the data format to skip marshaling / unmarshaling of the header record. Configure this parameter on
the primary record (for example, not the header or footer).
...
_FIX messages_, the model must contain a Header and Trailer classes linked
to the root message class which could be an `Order` class. This is not
mandatory but will be very helpful when you use camel-bindy in
...
| type | String |  | FIX | type is used to define the type of the message (for example, FIX, EMX, ...) (optional)

| version | String |  | 4.1 | version defines the version of the message (for example, 4.1, ...) (optional)
...
message looks like (https://github.com/apache/camel/blob/main/components/camel-bindy/src/test/data/fix/fix.txt[src/test/data/fix/fix.txt]) and the Order, Trailer,
Header classes
(https://github.com/apache/camel/blob/main/components/camel-bindy/src/test/java/org/apache/camel/dataformat/bindy/model/fix/simple/Order.java[src/test/java/org/apache/camel/dataformat/bindy/model/fix/simple/Order.java]).
...
If the tags/keys that we will put in the _FIX message_ must be sorted
according to a predefined order, then use the attribute `position` of the
...
In _FIX message_ of fixed length records, it is common to have different
...
Be careful, the one to many of bindy does not allow handling
repetitions defined on several levels of the hierarchy

The relation OneToMany ONLY WORKS in the following cases :

* Reading a _FIX message_ containing repetitive groups (= group of
tags/keys)
...
[NOTE]
====
The repetitive data concern the title of the book and its
...
FactoryRegistry in the registry in use (for example, Spring or JNDI).
...
then be defined in your registry, for example, your
Spring XML file:
...
The Camel route will pick up files in the inbox directory, unmarshal
...
list contains the model objects marshaled out of each line of
the CSV. The reason behind this is that _each line can correspond to
more than one object_. This can be confusing when you expect one
...
Exchange interface to do a characterset conversion of the inputstream received for unmarshaling.
In some producers, (for example, file-endpoint) you can define a characterset. This producer
can already do the character set conversion. Sometimes you need to remove this property or header from the
...
The second route will extract the pojos from the queue and marshalthe
