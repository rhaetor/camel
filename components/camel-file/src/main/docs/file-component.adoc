...
option, for example, by setting `fileName=thefilename`.
...
directory, disregarding the source directory layout (for example, to flatten out
...
Camel is also able to write files, that is, produce files.
