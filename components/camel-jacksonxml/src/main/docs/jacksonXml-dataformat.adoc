...
:description: Unmarshal XML payload to POJOs and back using XMLMapper extension of Jackson.
:since: 2.16
:supportlevel: Stable
:tabs-sync-option:
...
== Excluding POJO fields from marshaling

When marshaling a POJO to XML, you might want to exclude certain fields
...
Finally, use the Camel `JacksonXMLDataFormat` to marshal the above POJO
to XML.
...
== Unmarshaling from XML to POJO with dynamic class name

If you use Jackson to unmarshal XML to POJO, then you can now specify a
header in the message that indicates which class name to unmarshalto.
...
== Unmarshaling from XML to `List<Map>` or `List<POJO>`
...
when marshaling, you can configure this using the disableFeatures:
...
== Formatted XML marshaling (pretty-printing)

Using the `prettyPrint` option one can output a well-formatted XML while marshaling:
