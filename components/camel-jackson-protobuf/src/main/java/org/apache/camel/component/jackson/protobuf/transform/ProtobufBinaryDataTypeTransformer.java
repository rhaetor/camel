...
/**
 * Data type uses Jackson Protobuf data format to marshalgiven JsonNode in Exchange body to a binary (byte array)
 * representation. Uses given Protobuf schema from the Exchange properties when marshaling the payload (usually already
    ...
    public void transform(Message message, DataType fromType, DataType toType) {
        ...
        try {
            ...
            if (contentClass != null) {
                ...
                marshaled = Protobuf.mapper().writer().forType(contentType).with(schema)
                        .writeValueAsBytes(message.getBody());
            } else {
                marshaled = Protobuf.mapper().writer().forType(JsonNode.class).with(schema)
