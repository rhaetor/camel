...
[IMPORTANT]
====
Spring WS version 4.x does not support Axiom anymore (because Axiom does not support Jakarta JEE 9)
====
...
specified mapping-type (for example, a SOAP action, XPath expression). As a
...
pojo/xml marshaling using libraries such as JAXB.
