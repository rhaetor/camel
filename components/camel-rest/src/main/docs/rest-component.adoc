...
in the REST configuration, then the POJO will be marshaled from POJO to JSON before calling the REST service.

However, if you want to also perform binding for the response message (for example, what the REST service sends back, as response) you
would need to configure the `outType` option to specify what is the class name of the POJO to unmarshalfrom JSON to POJO.
