...
Now we can apply a stylesheet, for example:
...
Now we can apply again a stylesheet, for example:
...
[NOTE]
====
This transformation resulted in exactly the same json document as we used as input to the _json2xml_ conversion.
