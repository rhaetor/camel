...
marshaling of outbound payloads. Must override
...
marshaling of inbound payloads. Must override
...
If your encoders or decoders are not shareable (for example, they don't have the
...
a comma-separated list or contained in a list, for example:
...
You can get access to the `javax.net.ssl.SSLSession` if you for example, need to
