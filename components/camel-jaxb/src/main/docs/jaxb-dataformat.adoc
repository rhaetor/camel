...
JAXB is a Data Format which uses the JAXB XML marshaling standard to unmarshal an XML
...
<beans xmlns="http://www.springframework.org/schema/beans"
  ...
  <camelContext xmlns="http://camel.apache.org/schema/spring">
    <route>
      ...
      <to uri="direct:marshaled"/>
    </route>
    <route>
      <from uri="direct:marshaled"/>
...
== Partial marshaling / unmarshaling

JAXB 2 supports marshaling and unmarshaling XML tree fragments. By
...
need to unmarshal only part of the tree.

In that case, you can use partial unmarshaling. To enable this
behavior, you need set property `partClass` on the `JaxbDataFormat`. Camel will pass this class
to the JAXB unmarshaler. If `JaxbConstants.JAXB_PART_CLASS` is set as
one of the exchange headers, its value is used to override the `partClass` property
on the `JaxbDataFormat`.

For marshaling you have to add the `partNamespace` attribute with the `QName` of
...
`JaxbDataFormat` has a property named `fragment` which can set the `Marshaler.JAXB_FRAGMENT`
property on the JAXB Marshaler. If you don't want the JAXB Marshaler to generate
...
marshaling or unmarshaling. You can also set the Exchange property `Exchange.FILTER_NON_XML_CHARS`.
...
`Marshaler.JAXB_ENCODING` encoding property on the JAXB Marshaler.
...
When marshaling using xref:jaxb-dataformat.adoc[JAXB] or xref:jaxb-dataformat.adoc[SOAP] then
...
marshaling and unmarshaling from / to XML. You can use the prefix
...
The `JaxbDataFormat` supports to specify the `SchemaLocation` when marshaling the XML.
...
The JAXB marshaler requires that the message body is JAXB compatible,
for example, it is a `JAXBElement`, a java instance that has JAXB annotations, or
extends `JAXBElement`. There can be situations where the message body is
already in XML, for example, from a `String` type.

`JaxbDataFormat` has an option named `mustBeJAXBElement` which you can set to `false` to relax this check and
have the JAXB marshaler only attempt marshaling on `JAXBElement` (`javax.xml.bind.JAXBIntrospector#isElement` returns `true`).
In those situations, the marshaler will fall back to marshal the message body as-is.
