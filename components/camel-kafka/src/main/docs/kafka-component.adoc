...
Here is the minimal route you need to write messages to Kafka.
...
Each repository instance that uses the topic, (for example, typically on different machines running in parallel) controls its own consumer group, so in a cluster of 10 Camel processes using the same topic, each will control its own offset.
...
It is recommended to implement appropriate error handling mechanisms and patterns (that is, ; such as dead-letter queues), to prevent failed records from blocking processing progress.
