...
PostgreSQL can generate a huge number of empty transactions on certain operations (for example, `VACUUM`). These transactions can congest
...
this component to sync data from PostgreSQL to another database, make sure your operations are idempotent (for example, use `UPSERT`
