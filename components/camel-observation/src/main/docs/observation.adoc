...
By configuring the `ObservationRegistry` you can add behaviour to your observations such as metrics (for example, via `Micrometer`) or tracing (for example, via `OpenTelemetry` or `Brave`) or any custom behaviour.
