...
message body, that is, it should be of type `Bson` or convertible to `Bson`.
...
if the CamelMongoDbCriteria header is null the query object is extracted 
message body, that is, it should be of type `Bson` or convertible to
`Bson`., and
...
cursor every time it is regenerated, for example, a timestamp, a sequential ID,
