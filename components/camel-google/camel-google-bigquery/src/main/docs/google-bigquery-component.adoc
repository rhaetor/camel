...
The current implementation does not support querying BigQuery, that is, is a producer only.
...
partition decorators are specified. In that case, it will break it down to ensure data is written with the
...
will insert a single row, and a payload containing a list of maps will insert a row for each entry in the list.
...
That is, the following route will create tables and insert records sharded on a per-day basis:
...
An insert id can be set on the exchange with the header `GoogleBigQueryConstants.INSERT_ID` or by specifying
query parameter `useAsInsertId`. As an insert id need to be specified per row inserted the exchange header can't
be used when the payload is a list.
If the payload is a list the `GoogleBigQueryConstants.INSERT_ID` will
