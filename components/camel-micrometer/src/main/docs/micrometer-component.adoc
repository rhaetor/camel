...
That is, the URI parameter `tags=X=${header.Y}` would assign the current value of header `Y` to the key `X`.
...
If neither `increment` or `decrement` is defined, the value of the counter will
...
Both `increment` and `decrement` values are evaluated as `Simple` expressions with a Double result, for example:
...
`value` is evaluated as `Simple` expressions with a Double result, for example:
...
EventNotifiers can be added to the CamelContext, for example:
