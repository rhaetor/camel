...
The ControlBus component provides easy management of Camel applications based on the xref:controlbus-component.adoc[Control Bus] EIP pattern.
...
It is the same data you can get from JMX with the `dumpRouteStatsAsXml` operation on the `ManagedRouteMBean`.
...
For example, to stop a specific route, you can send a message to the `"controlbus:language:simple"` endpoint containing the following message:
