...
`S3Pointer` - return an S3 path (for example, `s3://bucket/path/`) pointing to the results:
...
S3 path (for example, `s3://bucket/path/`) in the body pointing to the results.  The path will also be set in a header
