...
Messages with the same correlation key are aggregated together, using an `AggregationStrategy`.
...
- To achieve synchronous aggregation, use an instance of `SynchronousExecutorService` for the
...
The completed Exchange sent out of the aggregator will contain the `List<V>` in
the message body.
...
If you need a persistent repository, then Camel provides numerous implementations, such as from the
...
indicate that the aggregated exchanges are complete, so they can be sent
out of the aggregator. Camel allows you to indicate completion in
various ways as follows:

* _completionTimeout_: Is an inactivity timeout in that is triggered if
...
when all the messages from the batch have been aggregated.
...
combine them in any way you like. It's basically the first that
...
If the `preComplete` method returns `true`, then the existing correlation group is
...
completion predicates / sizes / timeouts and so on, and complete the group.
...
completion predicates / sizes / timeouts and so on, and complete all the existing
previous group.
...
The aggregator provides a default implementation if no custom one has been
...
bean. To use a bean, then a convention must be followed:
...
`oldExchange`, and the reminder half is for the `newExchange`.
 Therefore, there must be an equal number of parameters, for example, 2, 4, 6, etc.
...
In the method below, we have only two parameters, so the first parameter is
...
In the method below, we have only four parameters, so the first parameter is
the body of the `oldExchange`, and the second is the `Map` of the
...
If we didn't do this, then on timeout the append method would normally not be
...
parameter type of the message bodies does not have to be the same.
