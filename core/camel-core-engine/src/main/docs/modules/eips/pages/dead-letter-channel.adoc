...
* using exponential backoff (that is, the time between retries increases using a backoff multiplier)
...
their sub-route (that is, the _split_ message); however, these EIPs have an option named `shareUnitOfWork` which allows
