...
The bean should be thread-safe in case concurrent threads are calling the bean at the same time.
...
look for it in the registry), you can provide the bean:
...
This is used throughout all the various xref:manual::bean-integration.adoc[Bean Integration] mechanisms in Camel.
