...
You can set `inheritNamsepaceTagName` property to `*` to include the preceding context in each token (that is, generating each token enclosed in its ancestor elements). It is noted that each token must share the same ancestor elements in this case.
