...
Doing so requires you to specify the plugin using its fully qualified name. For example to run the goal on
...
In particular, to use the validate plugin with transitive dependencies, *sources jars are needed*, for example:
